...
const validationRules = {
  login: {
    ...
    validate: {
      specialCharacters: (v) => (
        /^[A-Za-z0-9]+$/.test(v) ? null : 'Special characters are prohibited'),
    },
  },
  password: {
  ...
  email: {
    validate: {
      emailValidate: (v) => {
        if (v?.length) {
          return /^\S+@\S+\.\S+$/.test(v) ? null : 'Email is invalid';
        }
        return null;
      },
    },
          ...
          <form onSubmit={handleSubmit(handleLogin)}>
            <FormControl fullWidth error={!!errors.login}>
              ...
              <Input
                id="login-input"
                required
                aria-describedby="login-input-text"
                {...register('login', { ...validationRules.login })}
              />
              <FormHelperText id="login-input-text">
                {errors.login?.message || 'Enter login'}
              </FormHelperText>
            </FormControl>
